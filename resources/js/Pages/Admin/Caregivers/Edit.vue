...
const { form, processing, message, messageTone, submit, errorFor } = useApiForm({
    first_name: props.caregiver.first_name || '',
    last_name: props.caregiver.last_name || '',
    email: props.caregiver.email || '',
    phone: props.caregiver.phone || '',
    address_line_1: props.caregiver.address_line_1 || '',
    address_line_2: props.caregiver.address_line_2 || '',
    city: props.caregiver.city || '',
    state: props.caregiver.state || '',
    postal_code: props.caregiver.postal_code || '',
    country: props.caregiver.country || 'Nigeria',
    ...
    bio: props.caregiver.bio || '',
    status: props.caregiver.status || 'active',
});

const handleSubmit = async () => {
    try {
        await submit({
            url: `/caregivers/${props.caregiver.id}`,
            method: 'put',
        });
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
                ...
                <h1 class="text-2xl font-bold tracking-tight text-text">Edit Caregiver</h1>
                <p class="mt-1 text-sm text-text-muted">
                    Update caregiver profile and settings.
                </p>
            </div>
            <div class="flex gap-2">
                <Link :href="`/admin/caregivers/${caregiver.id}/availability`" class="btn-secondary shrink-0">
                    Manage Availability
                </Link>
                <Link :href="`/admin/caregivers/${caregiver.id}/attendance`" class="btn-secondary shrink-0">
                    Attendance
                </Link>
            </div>
        </div>
