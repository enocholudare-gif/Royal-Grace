...
export default function CheckIn({ booking }) {
    ...
    const getLocation = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            (error) => {
                ...
                setLocationStatus('error');
                setLocationError('Unable to retrieve your location. Please ensure location services are enabled in your browser settings.');
            },
    ...
    const submit = (e) => {
        ...
        if (locationStatus !== 'success') {
            alert('Please allow location access before checking in.');
            return;
        }
        post(`/caregiver/visits/${booking.id}/check-in`);
    };

    return (
        <>
            <Head title="Visit Check-In" />
            
            <div className="max-w-2xl mx-auto space-y-6">
                <div className="flex items-center gap-4">
                    <Link href={`/caregiver/visits/${booking.id}`} className="text-sm font-medium text-text-muted hover:text-brand-600 transition-colors">
                        &larr; Back to Visit Details
                    </Link>
                </div>

                <div className="card p-8 text-center border-t-4 border-brand-500">
                    ...
                    <h1 className="text-2xl font-bold text-text mb-2">Check-in at Client Location</h1>
                    <p className="text-text-muted mb-8">
                        You are about to start your visit with <span className="font-bold text-text">{booking.client?.user?.first_name} {booking.client?.user?.last_name}</span>. 
                        Your location will be recorded to verify attendance.
                    </p>
                    ...
                    <form onSubmit={submit}>
                        ...
                        >
                            {processing ? 'Processing Check-in...' : 'Confirm Check-in & Start Visit'}
                        </button>
                    </form>
                </div>
            </div>
...
CheckIn.layout = page => <CaregiverLayout title="Check-in" description="Start your visit">{page}</CaregiverLayout>;
